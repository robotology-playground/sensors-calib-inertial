...
% latex compliant motor name
dispMotorName = System.toLatexInterpreterCompliant(motorName);
...
switch frictionOrKtau
    case 'friction'
        ...
        Plotter.plot2funcTimeseriesYY(...
            figuresHandler,...
            ['Motor velocity and torque over time (' dispMotorName ')'],'motorVel_N_TorqFilt',...
        ...
        Plotter.plot2dDataNfittedModel(...
            figuresHandler,...
            ['Motor velocity to torque model (' dispMotorName ')'],'motorVel2torq',...
            dqMradG,tauMotorG,...
            [],[],...
            'Motor velocity (radians/s)','Motor torque (N.m)',...
            'Training data','');
    ...
    case 'ktau'
        pwmDutyCycle = ...
            data.parsedParams.(['pwm_' jointMotorCoupling.part '_state'])(motorIdx,:); % * jointMotorCoupling.fullscalePWMs{motorIdx}/100;
        
        % filtered Tau(time) & pwm(time)
        Plotter.plot2funcTimeseriesYY(...
            figuresHandler,...
            ['Motor PWM and torque over time (' dispMotorName ')'],'motorPWM_N_torqFilt',...
            time,pwmDutyCycle,time,tauMotorG,...
            'Motor PWM (duty cycle %)','Motor torque (N.m)');
        
        % filtered Tau VS pwm
        highlightHisteresis = true;
        Plotter.plot2dDataNfittedModel(...
            figuresHandler,...
            ['Motor PWM to torque model (' dispMotorName ')'],'motorPWM2torq',...
            pwmDutyCycle,tauMotorG,...
            [],[],...
            'Motor PWM (duty cycle %)','Motor torque (N.m)',...
            'Training data','',highlightHisteresis);
